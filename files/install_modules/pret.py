...
## Summon Installation module for: PRET
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/printers/PRET/console.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/printers/PRET/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/RUB-NDS/PRET.git","/redteam/printers/","PRET"):
            self.python.pip_install("colorama")
            self.python.pip_install("pysnmp")
