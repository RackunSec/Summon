...
## Summon Installation module for: TheHarvester
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/osint/theHarvester/requirements/base.txt" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/osint/theHarvester/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/laramies/theHarvester.git","/redteam/osint/","theHarvester"):
            if os.path.exists("/redteam/osint/theHarvester/requirements/base.txt"): ## This definitiely will be true because checks are done in git_clone()
                self.shell.run_cmd(["cp","/redteam/osint/theHarvester/requirements/base.txt","/redteam/osint/theHarvester/requirements.txt"])
                self.python.pip_reqs("theHarvester","/redteam/osint/theHarvester")
