...
## Summon Installation module for: Bighuge OSINT Tool
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/bbot" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/osint/bbot"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        if self.force: ## we are destroying all old artifacts:
            ...
            if len(self.badpaths)>0:
                ...
                for path in self.badpaths:
                    if os.path.exists(path):
                        self.shell.run_cmd(["rm","-rf",path]) ## Destroy local repo and binary in $PATH.
                if os.path.exists(self.install_path_check):
                        self.shell.run_cmd(["rm","-rf",self.install_path_check])
        ## Installation instructions below:
        ## --------------------------------

        if self.apps.git_clone("https://github.com/blacklanternsecurity/bbot.git","/redteam/osint/","bbot"):
            self.python.pip_install_dot("/redteam/osint/bbot") ## install it.
