...
## Summon Installation module for: Reaver
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/reaver" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/reaver-wps-fork-t6x/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/t6x/reaver-wps-fork-t6x.git","/redteam/wifi/","reaver-wps-fork-t6x"):
            os.chdir("/redteam/wifi/reaver-wps-fork-t6x/src")
            self.apps.apt_install(["build-essential","libpcap-dev"])
