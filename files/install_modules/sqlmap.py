...
## Summon Installation module for: SQLMap
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/web/sqlmap/sqlmap.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/sqlmap/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        self.apps.git_clone("https://github.com/sqlmapproject/sqlmap.git","/redteam/web/","sqlmap")
