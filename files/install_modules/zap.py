...
## Summon Installation module for: OWASP ZAP
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/web/ZAP_2.11.1/zap.sh" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/ZAP_2.11.1/","/redteam/web/zap.tgz"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        self.files.download_file(self.demon_repo+"zap.tgz","/redteam/web/zap.tgz",False)
        os.chdir("/redteam/web/")
        self.shell.run_cmd(["tar","vxzf","zap.tgz"])
