...
## Summon Installation module for: VSCode
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/share/code/bin/code" ## This file will exist when the application is properly installed.
        self.badpaths=["/usr/share/code/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        self.files.download_file(self.demon_repo+"vscode-1.72.2.deb","/tmp/vscode.deb",False)
        self.apps.dpkg_install("/tmp/vscode.deb")
        self.install_path_check="/usr/share/code/code"
