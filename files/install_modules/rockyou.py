...
## Summon Installation module for: Rockyou.txt
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/wordlists/rockyou.txt" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wordlists/rockyou.txt","/redteam/wordlists/rockyou.txt.gz"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        self.files.download_file(self.demon_repo+"rockyou.txt.gz","/redteam/wordlists/rockyou.txt.gz",True)
        os.chdir("/redteam/wordlists")
        self.shell.run_cmd(["gunzip","rockyou.txt.gz"])
