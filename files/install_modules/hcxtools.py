...
## Summon Installation module for: HCXTools
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/sbin/hcxpcaptool" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/hcxtools","/redteam/wifi/Hcxpcaptool"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/ZerBea/hcxtools.git","/redteam/wifi/","hcxtools"):
            os.chdir("/redteam/wifi/hcxtools")
            self.shell.run_cmd(["make"])
            self.shell.run_cmd(["make","install"])
        if self.apps.git_clone("https://github.com/warecrer/Hcxpcaptool.git","/redteam/wifi/","Hcxpcaptool"):
            os.chdir("/redteam/wifi/Hcxpcaptool")
            self.shell.run_cmd(["make"])
            self.shell.run_cmd(["cp","hcxpcaptool","/usr/local/sbin/hcxpcaptool"])
