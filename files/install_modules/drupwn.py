...
## Summon Installation module for: DruPWN
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/drupwn" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/drupwn"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/immunIT/drupwn.git","/redteam/web/","drupwn"):
            os.chdir("/redteam/web/drupwn")
            self.shell.run_cmd(["python3","setup.py","install"])
            self.python.pip_install_dot("/redteam/web/drupwn") ## still doesn't install in $PATH right :/
