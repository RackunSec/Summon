...
## Summon Installation module for: ZeroLogon
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/windows-domains/zerologon/set_empty_pw.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/windows-domains/zerologon/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/risksense/zerologon.git","/redteam/windows-domains/","zerologon"):
            self.apps.apt_install(["libffi-dev"]) ## We need this to compile.
            self.python.pip_reqs("Zerologon","/redteam/windows-domains/zerologon")
