...
## Summon Installation module for: APP_NAME
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/macchanger" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/macchanger/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        ##vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
        if self.apps.git_clone("https://github.com/alobbs/macchanger.git","/redteam/wifi/","macchanger"):
            self.apps.apt_install(["autoconf"]) ## this is required
            os.chdir("/redteam/wifi/macchanger")
            self.apps.apt_install(["texinfo"])
            self.shell.run_cmd(["./autogen.sh"]) ## Generate a config file
