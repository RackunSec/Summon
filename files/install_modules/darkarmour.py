...
## Summon Installation module for: DarkArmour
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/evasion/darkarmour/darkarmour.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/evasion/darkarmour/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/bats3c/darkarmour.git","/redteam/evasion/","darkarmour"):
            pass # That's it.
