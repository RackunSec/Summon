...
## Summon Installation module for: Nikto2
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/web/nikto/program/nikto.pl" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/nikto/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/sullo/nikto.git","/redteam/web/","nikto"):
            os.chdir("/redteam/web/nikto/program")
            self.shell.run_cmd(["git","checkout","nikto-2.5.0"])
