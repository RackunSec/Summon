...
## Summon Installation module for: SMBMap
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/windows-domains/smbmap/smbmap.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/windows-domains/smbmap/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/ShawnDEvans/smbmap.git","/redteam/windows-domains/","smbmap"):
            self.python.pip_reqs("SMBMap","/redteam/windows-domains/smbmap")
