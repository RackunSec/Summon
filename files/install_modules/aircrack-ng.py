...
## Summon Installation module for: Aircrack-NG
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/sbin/airmon-ng" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/aircrack-ng"] ## Destroy local repo and binary in $PATH.
    ...
    def install(self):
        ...
        if self.force: ## we are destroying all old artifacts:
            ...
            if len(self.badpaths)>0:
                print(f"{self.style.sub}{self.style.CMNT}Removing old installations from $PATH{self.style.RST}")
                for path in self.badpaths:
                    if os.path.exists(path):
                        self.shell.run_cmd(["rm","-rf",path]) ## remove what was there from apt or pip
        ...
        if self.apps.git_clone("https://github.com/aircrack-ng/aircrack-ng.git","/redteam/wifi/","aircrack-ng"):
            self.apps.apt_install(["build-essential","autoconf","automake","libtool","pkg-config","libnl-3-dev",
            "libnl-genl-3-dev","libssl-dev","ethtool","shtool","rfkill","zlib1g-dev","libpcap-dev","libsqlite3-dev",
            "libpcre3-dev","libhwloc-dev","libcmocka-dev","hostapd","wpasupplicant","tcpdump","screen","iw","usbutils"])
            os.chdir("/redteam/wifi/aircrack-ng/")
            self.shell.run_cmd(["autoreconf","-i"])
            self.shell.run_cmd(["./configure"])
            self.shell.run_cmd(["make"])
            self.shell.run_cmd(["make","install"])
            os.chdir(self.apps.current_dir)
        else:
            return False
