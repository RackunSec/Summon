...
## Summon Installation module for: EAPHammer
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/wifi/eaphammer/eaphammer" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/eaphammer/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/RackunSec/rackunsec-eaphammer.git","/redteam/wifi/","eaphammer"):
            self.python.pip_install("service_identity")
            self.shell.run_cmd(["python3","-m","pip install","pyopenssl","--upgrade"]) ## this requires an upgrade
            os.chdir("/redteam/wifi/eaphammer")
            self.shell.run_cmd(["python3","debian-setup.py","-y"]) ## install everything.
