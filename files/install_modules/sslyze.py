...
## Summon Installation module for: SSLyze
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/sslyze" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/sslyze"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/nabla-c0d3/sslyze.git","/redteam/web/","sslyze"):
            os.rename("/redteam/web/sslyze/dev-requirements.txt","/redteam/web/sslyze/requirements.txt") ## thanks.
            self.python.pip_reqs("SSLyze","/redteam/web/sslyze/")
            os.chdir("/redteam/web/sslyze") ## go here.
