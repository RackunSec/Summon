...
## Summon Installation module for: Wfuzz
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/wfuzz" ## This file will exist when the application is properly installed.
        self.badpaths=["/root/.local/bin/wfuzz","/usr/bin/wfuzz","/redteam/web/wfuzz"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/xmendez/wfuzz.git","/redteam/web/","wfuzz"):
            os.chdir("/redteam/web/wfuzz")
            self.shell.run_cmd(["apt","--purge","remove","python3-pycurl","-y"]) ##  remove this shit.
            self.python.pip_install("pycurl") ## Get what was supposed to be there in the first place.
            self.shell.run_cmd(["python3","setup.py","install"])
            self.python.pip_install_dot("/redteam/web/wfuzz")
