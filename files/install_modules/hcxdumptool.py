...
## Summon Installation module for: HCXDumpTool
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/bin/hcxdumptool" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/wifi/hcxdumptool"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        self.apps.apt_install(["libcurl4-openssl-dev","libssl-dev","pkg-config"])
        if self.apps.git_clone("https://github.com/ZerBea/hcxdumptool.git","/redteam/wifi/","hcxdumptool"):
            os.chdir("/redteam/wifi/hcxdumptool")
            self.shell.run_cmd(["make"])
            self.shell.run_cmd(["make","install"])
            os.chdir(self.apps.current_dir) ## go back - done
