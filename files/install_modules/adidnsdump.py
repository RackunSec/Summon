...
## Summon Installation module for: ADI DNS Dump
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/local/bin/adidnsdump" ## This file will exist when the application is properly installed.
        self.badpaths=["/usr/local/bin/adidnsdump","/redteam/windows-domains/adidnsdump"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/dirkjanm/adidnsdump.git","/redteam/windows-domains/","adidnsdump"):
            self.python.pip_install_dot("/redteam/windows-domains/adidnsdump")
