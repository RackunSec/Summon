...
## Summon Installation module for: Domain-Fronting-Lists
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/evasion/DomainFrontingLists/Azure.txt" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/evasion/DomainFrontingLists"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/vysecurity/DomainFrontingLists.git","/redteam/evasion/","DomainFrontingLists"):
            pass # That's it!
