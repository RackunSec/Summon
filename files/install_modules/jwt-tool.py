...
## Summon Installation module for: JWT_Tool
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/web/jwt_tool/jwt_tool.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/web/jwt_tool/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/ticarpi/jwt_tool.git","/redteam/web/","jwt_tool"):
            self.python.pip_install("termcolor")
            self.python.pip_install("cprint")
            self.python.pip_install("pycryptodomex")
            self.python.pip_install("requests")
