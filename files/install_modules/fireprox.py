...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/evasion/fireprox/fire.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/evasion/fireprox/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/ustayready/fireprox.git","/redteam/evasion/","fireprox"):
            self.python.pip_reqs("FireProx","/redteam/evasion/fireprox")
