...
## Summon Installation module for: Roast in the Middle
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/windows-domains/RITM/poetry.lock" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/windows-domains/RITM/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/Tw1sm/RITM.git","/redteam/windows-domains/","RITM"):
            self.python.poetry_install("/redteam/windows-domains/RITM/","Roast in the Middle")
