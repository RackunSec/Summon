...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/passwords/pypassgen/pypassgen/__main__.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/passwords/pypassgen/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/puzzlepeaches/pypassgen.git","/redteam/passwords/","pypassgen"):
            self.python.poetry_install("/redteam/passwords/pypassgen","PyPassGen")
