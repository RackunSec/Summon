...
## Summon Installation module for: Metasploit Framework
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/usr/bin/msfconsole" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/exploit/msfinstaller","/opt/metasploit-framework/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        self.files.download_file(
            ...
            "/redteam/exploit/msfinstaller",True)
        os.chmod("/redteam/exploit/msfinstaller",stat.S_IEXEC)
        self.shell.run_cmd(["/redteam/exploit/msfinstaller"])
    ...
    def uninstall(self):
        ...
        self.apps.apt_remove(["metasploit-framework"])
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
