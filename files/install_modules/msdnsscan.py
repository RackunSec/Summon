...
## Summon Installation module for: MS DNS Scan
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/osint/msdnsscan/msdnsscan.py" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/osint/msdnsscan"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/dievus/msdnsscan.git","/redteam/osint/","msdnsscan"):
            self.python.pip_reqs("MSDNSScan","/redteam/osint/msdnsscan")
