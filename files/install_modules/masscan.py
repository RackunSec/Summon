...
## Summon Installation module for: Masscan
...
class Application():
    def __init__(self,force):
        ...
        self.install_path_check ="/redteam/enumeration/masscan/bin/masscan" ## This file will exist when the application is properly installed.
        self.badpaths=["/redteam/enumeration/masscan/"] ## Destroy local repo (/redteam/(CATEGORY)/repo) and binary in $PATH.
        ##^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def install(self):
        ...
        if self.apps.git_clone("https://github.com/robertdavidgraham/masscan.git","/redteam/enumeration/","masscan"):
            os.chdir("/redteam/enumeration/masscan")
